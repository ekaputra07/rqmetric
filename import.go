...
func Import(filePath, profile string, re *regexp.Regexp, maxURLPartsLength int64) {
	...
	StartWorker(importID, re, maxURLPartsLength, wg, lineChan, resultChan)
	...
	fmt.Printf("\nFinished in %.2fs, your import output: rqmetric_output_%v.csv\n\n", time.Since(startTime).Seconds(), importID)
	fmt.Printf("Now you can view the report with command: `rqmetric -view rqmetric_output_%v.csv`\n\n", importID)
}
